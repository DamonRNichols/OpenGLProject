...
Emitter::Emitter() :
m_particles(nullptr), m_max_particles(0), m_alive_count(0), m_vertex_data(nullptr), m_index_data(nullptr)
{
	m_buffer = {};
}
Emitter::~Emitter()
{
	...
	glDeleteVertexArrays(1, &m_buffer.m_VAO);
	glDeleteBuffers(1, &m_buffer.m_VBO);
	glDeleteBuffers(1, &m_buffer.m_IBO);
}

void Emitter::Init(unsigned int a_max_particles, vec3 a_position, float a_emit_rate,
	...
	vec4 a_start_color, vec4 a_end_color)
{
	...
	m_particles = new Particle[a_max_particles];
	...
	glGenVertexArrays(1, &m_buffer.m_VAO);
	glGenBuffers(1, &m_buffer.m_VBO);
	glGenBuffers(1, &m_buffer.m_IBO);
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_buffer.m_IBO);
...
void Emitter::EmitParticles()
{
	...
	for (unsigned int i = 0; i < particles_to_emit && m_alive_count < m_max_particles; ++i)
	{
		...
		//change things here to morph the emitter shape
		vec4 n = glm::normalize(m_particles[m_alive_count].velocity);

		m_particles[m_alive_count].position.xyz = n.xyz * 4;
		m_particles[m_alive_count].velocity.xyz = m_particles[m_alive_count].velocity.xyz * -1;
...
void Emitter::Update(float dt)
{
	...
	for (unsigned int i = 0; i < m_alive_count; ++i)
	{
		m_particles[i].velocity.y += (1 * dt);
		m_particles[i].position += dt * m_particles[i].velocity;
